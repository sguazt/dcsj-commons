...
/**
 * A processor for the <em>TODO</em> annotation.
 *
 * This annotation processor follows the new JDK 6 annotation (JSR269) API.
 *
 * @author <a href="mailto:marco.guazzone@gmail.com">Marco Guazzone</a>
 */
@SupportedAnnotationTypes( "it.unipmn.di.dcs.common.annotation.TODO" )
public class TODOAnnotationProcessor extends AbstractProcessor
{
	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv)
	{
		if (!roundEnv.processingOver())
		{
			...
			{
				TODO todo = el.getAnnotation( TODO.class );
				if ( todo != null )
				{
					this.printMessage( todo, el );
				}
				TODOs todos = el.getAnnotation( TODOs.class );
				if ( todos != null )
				{
					for ( TODO t : todos.value() )
					{
						this.printMessage( t, el );
					}
				}
			}
		}
	...
	private void printMessage(TODO todo, Element el)
	{
		Messager m = this.processingEnv.getMessager();

		m.printMessage(
			NOTE,
			el.getSimpleName() + ">> " + todo.value(),
			el
		);
	}
}
